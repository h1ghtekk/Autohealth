            ...
            while (true) {
                try {
                    ...
                    if (bytes > 0) {
                        ...
                        // Проверяем наличие символа конца сообщения
                        int endIndex = msg.indexOf(">");
                        while (endIndex != -1) {
                            String message = msg.substring(0, endIndex + 1);
                            mBTHandler.obtainMessage(MainActivity.MESSAGE_READ, bytes, -1, message).sendToTarget();
                            msg.delete(0, endIndex + 1);
